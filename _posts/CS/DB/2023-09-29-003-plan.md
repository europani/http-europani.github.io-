...
3. 인덱스 고유 스캔(Index Unique Scan) : PK나 Unique Index로 테이블에 접근하는 방식
  ...
  - <span style="color: red;">가장 효율적인 스캔 방식</span>
...
5. 인덱스 병합 스캔(Index Merge Scan) : 테이블 내의 생성된 인덱스를 통합해서 스캔하는 방식
  ...
  - 물리적으로 분리된 인덱스를 각각 수행하므로 **비효율적**이기에 하나의 인덱스로 통합하여 튜닝하거나 SQL문을 수정해 하나의 인덱스만 사용하도록 튜닝해야 한다
...
- SIMPLE : union이나 내부 쿼리 없는 SELECT문
