...
## OAuth2 + JWT flow
![image](https://user-images.githubusercontent.com/48157259/149644276-987ed410-0482-478a-892b-3334b6b0bcda.png)
...
- Authorization 과정에서 Cookie를 사용하기 위한 클래스
- `oauth2_auth_request` 쿠키 : 해당 Authorizaion request의 고유 아이디를 담는다
- `redirect_uri` 쿠키 : 해당 Authorization request시 파라미터로 넘어온 redirect_uri를 담는다. 이 쿠키는 나중에 `applcation.yml`의 `authorizedRedirectUris`와 일치하는지 확인시 사용된다
```
...
- 로그인에 성공하면 JWT를 생성한 다음 `authorizedRedirectUris`로 client에게 전송한다
