...
- <span style="color:red">JPQL은 만들어진대로 단지 SQL로 변환만 할 뿐이지 최적화같은 것은 수행하지 않는다</span>  
...
#### Distinct
- 일대다(1:N) 조인의 경우 DB의 row가 증가한다. 그 결과 같은 엔티티의 조회수도 증가하게 된다.
- SQL에 `DISTINCT`를 추가하면 JPA는 중복된 엔티티를 1번만 가져온다
...
1. ToOne(ManyToOne, OneToOne) 관계를 모두 페치조인
    - ToOne 관계는 row수를 증가시키지 않아서 영향이 없다
2. 컬렉션은 페치조인이 아닌 지연 로딩으로 조회하고
3. 지연 로딩 최적화로 배치사이즈 설정 (hibernate.default_batch_fetch_size 또는 @BatchSize 설정)
