...
- show-sql : 실행되는 쿼리 콘솔에 출력
- properties.hibernate.format_sql : 콘솔창에 출력되는 쿼리를 가독성 좋게 포맷팅
- hibernate.ddl-auto : 데이터베이스 초기화전략
  - none : 아무것도 하지 않음
  - ~~create~~ : 기존 테이블 삭제 후 생성
  - ~~create-drop~~ : 기존 테이블 삭제 후 생성, 연결 종료시 테이블 삭제
  - ~~update~~ : 수정된 스키마만 적용
  - validate : 엔티티와 테이블 정상 맵핑 확인
  
  - <span style="color:red">운영환경에서 절대로 `create`, `create-drop`, `validate`를 사용해서는 안된다</span>
  - 개발환경에서는 `update` 사용
