...
![](https://user-images.githubusercontent.com/48157259/172301889-ea3489e8-4490-4918-b5cd-c6af91a81f38.png)
1. 스프링은 빈 객체를 생성한다
2. 생성된 빈 객체를 `스프링 컨테이너`에 등록하기 전에 `빈 후처리기`에 전달한다
3. 모든 `Advisor`를 조회해 해당 빈이 적용 대상인지 확인한다
...
- `@Aspect` : 공통 기능의 클래스(Aspect) 설정
  - `AnnotationAwareAspectJAutoProxyCreator`에 의해 어드바이저로 변환되어 저장
...
- 하지만 프록시를 거치지 않고 대상객체를 직접 호출하게 된다면 공통 로직을 거치지 않게 된다
